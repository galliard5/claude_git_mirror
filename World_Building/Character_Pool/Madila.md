---
name: Madila
keywords: [dnd35, kobold, half-dragon-gold, monk, starscream-campaign, galliard, saturday-morning, lg]
description: Galliard's kobold Half-Dragon (Gold) monk 4 named Madila. LG, ambidextrous, red scales, yellow eyes. Weapon Finesse + Improved Natural Armor. Monk bonus feats: Improved Grapple + Combat Reflexes. Has a MapTool token (Madila.rptok) also in the import folder.
---

# Madila

**System:** D&D 3.5e (PCGen)
**Player:** Galliard
**File:** `Starscream saturday morning kobold monk.pcg`
**Campaign:** Starscream (Saturday morning session — implies a recurring, lighter-toned game)

## Identity
- **Race:** Kobold + Half-Dragon (Gold) template | **Gender:** Female
- **Height:** 38 in (3'2") | **Weight:** 43 lb | **Size:** Small
- **Skin:** red (scales) | **Eyes:** yellow
- **Alignment:** Lawful Good
- **Handedness:** Ambidextrous
- **XP:** 21,000
- **Languages:** Draconic only

## Attributes
STR 12 | **DEX 15** | CON 14 | INT 12 | **WIS 16** | CHA 10

The WIS 16 for a monk is excellent — WIS governs the Monk's AC bonus and Will saves. The Half-Dragon template adds natural armor, fire immunity, and natural weapons.

## Classes
Monk 4

## Racial Template: Half-Dragon (Gold)
- Fire breath weapon (instead of acid for black dragons)
- Claw × 2 + Bite natural weapons (Small size: Claw 1d3, Bite 1d4)
- Natural armor bonus
- Darkvision, Low-Light Vision
- Fire immunity (Gold dragon lineage)

## Feats
**Weapon Finesse** — DEX to attack rolls with light/unarmed attacks
**Improved Natural Armor** — +1 natural armor above the Half-Dragon bonus

**Monk Bonus Feats:**
- **Improved Grapple** (1st) — +4 grapple, no attack of opportunity
- **Combat Reflexes** (2nd) — extra attacks of opportunity equal to DEX bonus

## Skills (7 ranks each)
Balance | Hide | Jump | Move Silently | Tumble

## Equipment
| Item | Notes |
|------|-------|
| Bracers of Armor +1 | Magical armor bonus for a monk who can't wear armor |
| Ring of Feather Falling | |
| Wand (Cure Light Wounds) | |
| Backpack + Bedroll | |
| 47 gp | |

A monk's Unarmored AC bonus makes magical bracers their primary defensive item. The ring of feather falling is practical for a jumping/tumbling monk who gets knocked off things.

## Natural Weapons
Claw (Primary) × 2 | Bite (Secondary) | Flurry of Blows (Monk)

The Flurry of Blows is listed as an equipment item ("Flurry of Blows," quantity 0) — a PCGen artifact for the monk's full-attack mode.

## MapTool Token — Advanced Version
`Madila.rptok` confirms she was actively played in VTT. The token represents a **significantly higher-level** version of Madila than the PCGen sheet (level 4), showing substantial development:

### VTT Skill Bonuses
| Skill | Total |
|-------|-------|
| Move Silently | **+22** |
| Tumble | **+22** |
| Climb | **+25** (+5 from Diana staff) |
| Spot | +15 |
| Hide | +11 |
| Balance | +9 |
| Search | +3 |

### VTT Attack Bonuses
Quarterstaff: **+20** standard / **+24+10** when flanking (the +10 is from the Chameleon Cloak). This implies BAB well into double digits — likely level 12+.

Standard monk unarmed damage: **2d6+2+STRbonus** — for a Small creature on the monk damage table, 2d6 is approximately monk level 16+.

### Additional Powers (VTT only, not in PCGen sheet)
| Power | Notes |
|-------|-------|
| **Werebear** | Werebear Off toggle macro — she acquired lycanthropy |
| **Spellreaving Fist** | Dispels magic on hit; uses level+6 modifier |
| **Dayburst Fist** | Special strike |
| **Fiery Fists** | Fire-damage unarmed attacks (Half-Dragon synergy) |
| **Eviscerating Strike** | Called precision strike |
| **Eye Gouge** | Called shot to the eye |
| **Nerve Strike** | Targeted strike |
| **refresh chakra** | Chi/energy recovery ability |

### Full Equipment (VTT)
| Item | Notes |
|------|-------|
| **+2 Axiomatic Ki Strike quarterstaff "Diana"** | Also a +2 Returning Ki Strike javelin when thrown. Axiomatic: 2d6 vs chaotic. Can grow/shrink 1 inch to 20 feet (standard action). +5 Jump. Free trip attempt on any hit. Marked with the name Diana. |
| **+1 Sling of Undead Bane** | "Holy sling" in VTT |
| **+5 Leather Armor** | |
| **+1 Bracers of Armor** | |
| **Chameleon Cloak** | +10 to attack bonus when flanking |
| Ring of Featherfall | Same as PCGen |
| Ring of Force Shield | New — not on PCGen sheet |
| Wand of Invisibility | |
| **Wand of Cure Moderate Wounds** | 45 charges, 2d8+3 |
| **Marvelous Pigments** | Creates objects from painted surfaces |

### Werebear Note
The Werebear lycanthropy is the most significant addition beyond the PCGen sheet. As a werebear, Madila gains a powerful hybrid/animal form with bear natural weapons and STR bonuses. Combined with her monk unarmed attacks and Half-Dragon breath, she has three distinct physical combat modes: monk strikes, dragonborn breath, and bear form. The "Werebear Off" macro label suggests she can toggle between forms in the VTT interface.

## Notes
"Starscream saturday morning kobold monk" as a filename is the most evocative filename in the entire batch. It suggests a campaign called Starscream, played on Saturday mornings, involving a kobold monk. The "saturday morning" framing implies a lighter, more fun tone — a deliberate aesthetic choice, like a children's cartoon premise taken seriously.

Madila is LG, which is the required alignment for monks. A gold half-dragon kobold LG monk named Madila who plays on Saturday mornings with her claws, breath weapon, and Improved Grapple is exactly the kind of character concept that plays seriously in a game that doesn't take itself too seriously.

Combat Reflexes + Improved Grapple at level 4 means she can lock down a significant area: multiple AoO from DEX, and those AoO can be grapples. Anyone who tries to move past her in a corridor is likely getting grabbed.
