...
# =============================================================================
# 5b. CLAUDE-OPTIMIZED HEADER EMBEDDING
# =============================================================================
# Mirrors the directory_index.md pattern: a tiny fixed-format index header at
# the top says exactly how many lines the Claude-readable description spans,
# so a tool-based partial read can grab just the small description without
# loading the rest of the SVG into context.
#
# File layout produced by embed_claude_header():
#
#   line 1:  <?xml version="1.0" encoding="UTF-8"?>
#   line 2:  <!--
#   line 3:  CLAUDE_MAP_INDEX
#   line 4:  schema_version: 1
#   line 5:  source: <wxx filename>
#   line 6:  generated: <iso date>
#   line 7:  claude_section_end: <line number where description ends>
#   line 8:  -->
#   line 9:  <!--
#   line 10: CLAUDE_MAP_DESCRIPTION
#   line 11+: ...the Markdown description from wxx_to_claude.build_claude_doc()...
#   line N:  -->
#   line N+1: <svg xmlns="..."> ...
#
# To consume just the Claude section: read 8 lines, parse claude_section_end,
# read up to that line number. The SVG body never enters context.

def embed_claude_header(svg_text: str, claude_doc: str, source_filename: str = '') -> str:
    """Wrap an SVG with a Claude-readable header block at the top.

    The output is still a valid SVG that any renderer will display normally
    (XML comments before the root element are permitted). For tool-based
    consumption, partial-read the header to find claude_section_end, then
    read up to that line for just the description.
    """
    import datetime
    today = datetime.date.today().isoformat()

    # Strip the SVG's own XML declaration if present — we'll re-add ours.
    body = svg_text
    if body.startswith('<?xml'):
        body = body.split('\n', 1)[1] if '\n' in body else body

    # XML comments forbid '--' (double-hyphen) anywhere inside them. Markdown
    # table separators ('|-------|---------|') contain runs of hyphens and would
    # break the embedded comment. Replace '--' with '==' in the description —
    # markdown table syntax accepts '=' as a valid separator character, and
    # nothing else in our description text relies on consecutive hyphens.
    ...
    #   8: ==>     (line 8 closes the index using == sentinel; see note below)
    #   9: <!==
    #   10: CLAUDE_MAP_DESCRIPTION
    #   11..(10+len(desc_lines)): description lines
    #   11+len(desc_lines): ==>
    #
    # Note: lines 8, 9, and the final closing line use '==>' / '<!==' as
    # markers because we're INSIDE one big XML comment that opens at line 2
    # and closes at the description's "-->" terminator. The sentinels are
    # cosmetic separators, not actual XML comment delimiters.
    ...
    header_lines = [
        ...
        '==>',                          # cosmetic separator, NOT a comment terminator
        '<!==',                         # cosmetic separator
        'CLAUDE_MAP_DESCRIPTION',
    ]
    header_lines.extend(desc_lines)
    header_lines.append('-->')          # actual XML comment terminator
...
def main():
    ...
    ap.add_argument('--no-claude-header', action='store_true',
                    help='Skip the Claude-readable description header at the top of the SVG')
    args = ap.parse_args()
    ...
    if not args.no_claude_header:
        # Build the Claude-readable description and embed it as a header comment.
        try:
            from wxx_to_claude import build_claude_doc_from_map
            claude_doc = build_claude_doc_from_map(wmap, source_label=os.path.basename(args.input))
        except ImportError:
            # Fallback: re-parse via the file path
            from wxx_to_claude import build_claude_doc
            claude_doc = build_claude_doc(args.input)
        svg = embed_claude_header(svg, claude_doc, source_filename=os.path.basename(args.input))
        desc_line_count = len(claude_doc.splitlines())
        print(f'  embedded Claude header ({desc_line_count} lines of description)')
