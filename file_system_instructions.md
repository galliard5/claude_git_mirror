...
==========================
**Filesystem Tools (11):**
- Read: filesystem:read_file, filesystem:read_multiple_files
...
1. After preloading tools (see above), `filesystem:read_file` on `/corpus/file_system_instructions.md`
2. The YAML frontmatter at the top has `last_edited_utc`. Compare against the project-instructions copy in your context.
...
**Startup procedure:**
1. `filesystem:read_file` on `/corpus/index/directory_index.md` — loads the whole file: YAML header plus the compressed Claude section.
...
4. Execute batch write → verify each file with `filesystem:read_file`
...
**CRITICAL:** Always `filesystem:read_file` immediately before using `filesystem:edit_file`. Never rely on remembered content — `edit_file` requires exact string matching.
...
1. Immediately verify with `filesystem:read_file` — check the YAML frontmatter at the top and that the file ends as expected.
...
```
filesystem:write_file(content, path)
filesystem:read_file(path)  // Verify frontmatter at top, completion at bottom
```
